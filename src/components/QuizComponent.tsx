      ...
      <div className="space-y-3">
        {quizData.options.map((option, index) => (
          <button
            ...
            aria-label={`Quiz option ${index + 1}: ${option}`}
            aria-pressed={selectedOption === option}
            className={`w-full text-left p-6 sm:p-4 rounded-lg border-2 transition-all duration-200 transform hover:scale-105 min-h-[44px] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
